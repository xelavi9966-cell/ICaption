# themes/dark.py
import tkinter as tk
from tkinter import ttk

def apply_dark(root: tk.Tk, style: ttk.Style):
    try:
        style.theme_use("clam")
    except Exception:
        pass

    style.configure(".", background="#1e1e1e", foreground="#e6e6e6")
    style.configure("TFrame", background="#1e1e1e")
    style.configure("TLabel", background="#1e1e1e", foreground="#e6e6e6")
    style.configure("TButton", padding=6)
    style.configure("TCombobox", padding=4)
    style.theme_use("clam")

    bg = "#1e1e1e"
    fg = "#e7e7e7"
    field_bg = "#2a2a2a"
    border = "#3a3a3a"

    root.configure(bg=bg)

    style.configure(".", background=bg, foreground=fg)
    style.configure("TFrame", background=bg)
    style.configure("TLabel", background=bg, foreground=fg)
    style.configure("TCheckbutton", background=bg, foreground=fg)

    style.configure("TButton", padding=(10, 6))
    style.map("TButton",
              foreground=[("disabled", "#777777")],
              background=[("active", "#2a2a2a")])

    style.configure("TEntry", fieldbackground=field_bg, bordercolor=border)
    style.configure("TCombobox", fieldbackground=field_bg)

    style.configure("TSeparator", background=border)

    # ===== Custom Checkbutton (RED filled, no tick) via images =====
    if not hasattr(root, "_theme_imgrefs"):
        root._theme_imgrefs = {}

    def _mk_box(name: str, size: int, bgc: str, fillc: str | None, borderc: str):
        img = tk.PhotoImage(master=root, width=size, height=size)
        img.put(bgc, to=(0, 0, size, size))
        img.put(borderc, to=(0, 0, size, 1))
        img.put(borderc, to=(0, size-1, size, size))
        img.put(borderc, to=(0, 0, 1, size))
        img.put(borderc, to=(size-1, 0, size, size))
        if fillc:
            img.put(fillc, to=(1, 1, size-1, size-1))
        root._theme_imgrefs[name] = img
        return img

    size = 16
    unchecked = _mk_box("cb_dark_unchecked", size, bg, None, "#a0a0a0")
    checked = _mk_box("cb_dark_checked", size, bg, "#8b1a1a", "#a0a0a0")  # тёмно-красный

    #style.element_create(
    #    "Red.Checkbutton.indicator",
    #    "image",
    #    unchecked,
    #    ("selected", checked),
    #    sticky="w"
    #)

    #style.layout(
    #    "Red.TCheckbutton",
    #    [
    #        ("Checkbutton.padding", {
    #            "children": [
    #                ("Red.Checkbutton.indicator", {"side": "left"}),
    #                ("Checkbutton.label", {"side": "left", "padx": 6})
    #            ]
    #        })
    #    ]
    #)

    #style.configure("Red.TCheckbutton", background=bg, foreground=fg)
    # ===== Custom Checkbutton style: red filled, no tick (safe layout clone) =====
    if not hasattr(root, "_theme_imgrefs"):
        root._theme_imgrefs = {}

    def _mk_box(name: str, size: int, bgc: str, fillc: str | None, borderc: str):
        img = tk.PhotoImage(master=root, width=size, height=size)
        img.put(bgc, to=(0, 0, size, size))
        img.put(borderc, to=(0, 0, size, 1))
        img.put(borderc, to=(0, size - 1, size, size))
        img.put(borderc, to=(0, 0, 1, size))
        img.put(borderc, to=(size - 1, 0, size, size))
        if fillc:
            img.put(fillc, to=(1, 1, size - 1, size - 1))
        root._theme_imgrefs[name] = img
        return img

    def _replace_indicator(lspec, new_elem: str):
        out = []
        for elem_name, opts in lspec:
            if elem_name == "Checkbutton.indicator":
                elem_name = new_elem
            new_opts = dict(opts) if isinstance(opts, dict) else opts
            if isinstance(new_opts, dict) and "children" in new_opts and isinstance(new_opts["children"], list):
                new_opts["children"] = _replace_indicator(new_opts["children"], new_elem)
            out.append((elem_name, new_opts))
        return out

    size = 16
    unchecked = _mk_box("icap_cb_dark_unchecked", size, bg, None, "#a0a0a0")
    checked   = _mk_box("icap_cb_dark_checked",   size, bg, "#8b1a1a", "#a0a0a0")  # тёмно-красный

    indicator_elem = "ICap.Dark.indicator"
    if indicator_elem not in style.element_names():
        style.element_create(indicator_elem, "image", unchecked, ("selected", checked), sticky="w")

    base_layout = style.layout("TCheckbutton")
    new_layout = _replace_indicator(base_layout, indicator_elem)

    style.layout("ICap.Dark.TCheckbutton", new_layout)
    style.configure("ICap.Dark.TCheckbutton", background=bg, foreground=fg)